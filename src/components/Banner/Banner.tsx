...
const BannerTypes = cva(
    "flex items-center max-h-12 p-3 overflow-hidden gap-1 w-full",
    {
        variants: {
            variant: {
    ...
    ({ icon, children, className, variant, ...props }, ref) => {
        return (
            <div className={cn(BannerTypes({ variant, className }))} ref={ref}>
                ...
                <div className="overflow-x-clip text-ellipsis font-medium whitespace-nowrap">
                    {children}
                </div>
            </div>
        );
