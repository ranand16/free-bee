...
const Tag = forwardRef<HTMLInputElement, InputFieldProps>(
  ({ label, href, className, variant, removeButtonLabel, ...props }, ref) => {
    return (
      ...
      >
        {href ? (
          <a href={href} className="text-sm whitespace-nowrap text-ellipsis max-w-[180px] px-[4px] py-[2px] no-underline hover:underline text-brand-bold hover:bg-default-bold-hovered">
            {label}
          </a>
        ) : (
